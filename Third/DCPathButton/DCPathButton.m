...
- (void)configureViewsLayout
{
    ...
    self.foldCenter = CGPointMake(self.bloomSize.width / 2, self.bloomSize.height - 40);
...
//    rotationAnimation.values = @[@(0), @(M_PI), @(M_PI * 2)];
     rotationAnimation.values = @[@(0), @(0), @(0)];
...
- (void)pathCenterButtonBloom
{
    ...
    [UIView animateWithDuration:0.1575f
                     animations:^{
                         _pathCenterButton.transform = CGAffineTransformMakeRotation(-0.25f * M_PI);//  0.75
                     }];
...
//    rotationAnimation.values = @[@(0.0), @(- M_PI), @(- M_PI * 1.5), @(- M_PI * 2)];
    rotationAnimation.values = @[@(0.0), @(0.0), @(0.0), @(0.0)];
